...
def scraping(url, process_id):    
    ...
    with sync_playwright() as p:
        ...
        print(f"PID_{os.getpid()} (Process_{process_id}) starts for task: {e_code} - <{holding_date_ele}>")                  
        ...
        print(f"PID_{os.getpid()} (Process_{process_id}) parsing data: {etf['etf_name']}({etf['etf_code']})")                                                   
...
# Execute tasks
def worker(task_queue, result_queue, process_id):
    while not task_queue.empty():
        try:            
            task = task_queue.get(block=False)
        except Exception:
            break        
        result = scraping(task, process_id)                             
...
def main():    
    ...
    num_processes = len(etfs)
    processes = []
    for i in range(num_processes):
        process = multiprocessing.Process(target=worker, args=(task_queue, result_queue, i + 1))
...
if __name__ == '__main__':
    start = time.perf_counter()
    output_path = "outputs/crawl_mps.json"
    this_pid = os.getpid() 
    print("-"*30)  
    print("PID:", this_pid, "\n") 
    main()    
